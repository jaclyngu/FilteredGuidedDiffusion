...
def main():
    ...
    opt.schedule_fn = lambda x: int(x<=opt.t_start and x>opt.t_end)
    ...
    if opt.target_img:
        ...
        sampler.sigmas = [opt.spatial_std, opt.spatial_std, opt.value_std]      
             
        init_image = load_img(opt.target_img).to(device)
        ...
        cbm = acimops.bilateral.getCrossBilateralMatrix(sampler.target_np.astype('float'), \
                                                        sampler.sigmas)
        sampler.cbm = torch.Tensor(cbm).unsqueeze(0).repeat((ch, 1,1)).to(device)
