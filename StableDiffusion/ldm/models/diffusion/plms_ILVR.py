...
class PLMSSampler(object):
    ...
    def plms_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            img, pred_x0, e_t, pseudo_mean = outs
...
#                     );
            ...
            if FGD.use_FGD:
                with torch.no_grad():
#                     print(i, step, index)
                    guide_noise = self.stochastic_encode(FGD.xT_latent, torch.full((b,), index, device=device, dtype=torch.long))
                    img = FGD.Filter(self, img, guide_noise, ddim_use_original_steps, index).expand(*pred_x0.size())
...
#         return img
    ...
    def p_sample_plms(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if len(old_eps) == 0:
            ...
            x_prev, pred_x0, pseudo_mean = get_x_prev_and_pred_x0(e_t, index)
        ...
        x_prev, pred_x0, pseudo_mean = get_x_prev_and_pred_x0(e_t_prime, index)

        return x_prev, pred_x0, e_t, pseudo_mean
